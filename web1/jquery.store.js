(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "Clothes-"; // Prefix string to be prepended to the cart's name in the session storage
			this.cartName = this.cartPrefix + "cart"; // Cart name in the session storage
			this.wishName = this.cartPrefix + "wishlist";
			...
			this.$formAddToWish = this.$element.find( "form.add-to-wish" ); // Forms for adding wishlist items to the cart
			this.$formWish = this.$element.find("#wish-list"); //Wishlist form
			...
			this.$WishListActions=this.$element.find("#wish-list-actions");
			this.$emptyWishBtn = this.$WishListActions.find( "#empty-wish" ); // Empty Wishlist button
			this.$userDetails = this.$element.find( "#user-details-content" ); // Element that displays the user information
			this.$paypalForm = this.$element.find( "#paypal-form" ); // PayPal form
			
			
			this.currency = "Rs:"; // HTML entity of the currency to be displayed in the layout
			this.currencyString = "₹"; // Currency symbol as textual string
			this.paypalCurrency = "EUR"; // PayPal's currency code
			this.paypalBusinessEmail = "golthirithvik78@email.com"; // Your Business PayPal's account email address
		...
		createWish: function() {
			if( this.storage.getItem( this.wishName) == null) {
				var wish = {};
				wish.items = [];

				this.storage.setItem( this.wishName, this._toJSONString( wish ) );
			}
		},
		...
		deleteProduct: function() {
			...
			if( this.$formWish.length ) {
				var wish = this._toJSONObject( this.storage.getItem( this.wishName ) );
				var items = wish.items;
				...
				$( document ).on( "click", ".pdelete a", function( e ) {
					...
					newItems = items;
					var updatedWish = {};
					updatedWish.items = newItems;

					self.storage.setItem( self.wishName, self._toJSONString( updatedWish ) );
					$( this ).parents( "tr" ).remove();
					});
		...
		displayWish: function() {
			if( this.$formWish.length ) {
				var wish = this._toJSONObject( this.storage.getItem( this.wishName ) );
				var items = wish.items;
				var $wishBody = this.$formWish.find( "tbody" );
				
				if( items.length > 0 ) {
				
					for( var j = 0; j < items.length; ++j ) {
						var wishItem = items[j];
						var wishProduct = wishItem.product;
						var wishPrice = this.currency + " " + wishItem.price;
					
						
						var wishHtml = "<tr><td class='pname'>" + wishProduct + "</td>";
						wishHtml += "<td class='pprice'>" + wishPrice + "</td><td class='pdelete'><a href='' data-product='" + wishProduct + "'>&times;</a></td></tr>";

						$wishBody.html( $wishBody.html() + wishHtml );
					}
				} else {
					$wishBody.html( "" );	
				}
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						var qty = item.qty;
						var html = "<tr><td class='pname'>" + product + "</td>" + "<td>"+qty+"</td>";
		...
		emptyWish: function() {
			var self = this;
			if( self.$emptyWishBtn.length ) {
				self.$emptyWishBtn.on( "click", function() {
					self._emptyWish();
				});
			}
		},										
		
		//  Adds items to the Wish List.
		handleAddToWishForm: function() {
			var self = this;
			self.$formAddToWish.each(function() {
				var $form = $( this );
		...
		_emptyWish: function() {
			this.storage.removeItem( this.wishName);
		},
		...
		_addToWish: function( values ) {
			var wish = this.storage.getItem( this.wishName );
			
			var wishObject = this._toJSONObject( wish );
			var wishCopy = wishObject;
			var items = wishCopy.items;
			items.push( values );
			
			this.storage.setItem( this.wishName, this._toJSONString( wishCopy ) );
		},
